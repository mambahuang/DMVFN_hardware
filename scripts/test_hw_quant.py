...
# hw_quant: weight×256 INT MAC, Q11.10 flow, fixed-pt bilinear, linear sigmoid MASK_SHIFT=7
...
parser.add_argument('--val_datasets', type=str, nargs='+', default=['CityValDataset'],
                    help='[CityValDataset,KittiValDataset,VimeoValDataset,DavisValDataset]')
...
setup_logger('base', log_path, 'test_hw_quant', level=logging.INFO, screen=True, to_file=True)
logger = logging.getLogger('base')


def test(model, save_img=False):
    nr_eval = 0
    print('Start testing (hw_quant mode)...')
    ...
    for dataset_name in args.val_datasets:
        ...
        evaluate(model, val_data, dataset_name, nr_eval, save_img)


def evaluate(model, val_data, name, nr_eval, save_img):
    save_img_path = './save_img/test_hw_quant_{}/{}'.format(name, exp)
    ...
    if name in ("CityValDataset", "KittiValDataset", "DavisValDataset"):
        with torch.no_grad():
            ...
            for i, data in tqdm(enumerate(val_data), desc="Processing", total=num):
                ...
                for j in range(5):
                    psnr = -10 * math.log10(
                        torch.mean((gt[j+4] - pred[j]) * (gt[j+4] - pred[j])).cpu().data)
                    ssim_val    = ssim(gt[j+4:j+5], pred[j:j+1],
                                       data_range=1.0, size_average=False)
                    ms_ssim_val = ms_ssim(gt[j+4:j+5], pred[j:j+1],
                                          data_range=1.0, size_average=False)
                    x = ((gt[j+4:j+5] - 0.5) * 2.0).clone()
                    y = ((pred[j:j+1]  - 0.5) * 2.0).clone()
            ...
            psnr_score_mine   /= num
            ssim_score_mine   /= num
            msssim_score_mine /= num
            lpips_score_mine  /= num

            for i in range(5):
                logger.info(
                    '%d             ' % nr_eval + name +
                    '  psnr_%d     '    % i + '%.4f' % (sum(psnr_score_mine[:(i+1)])   / (i+1)) +
                    '  ssim_%d     '    % i + '%.4f' % (sum(ssim_score_mine[:(i+1)])   / (i+1)) +
                    '  ms_ssim_%d     ' % i + '%.4f' % (sum(msssim_score_mine[:(i+1)]) / (i+1)) +
                    '  lpips_%d     '   % i + '%.4f' % (sum(lpips_score_mine[:(i+1)])  / (i+1)))

    elif name == "VimeoValDataset":
        with torch.no_grad():
            lpips_score_mine  = np.zeros(1)
            psnr_score_mine   = np.zeros(1)
            msssim_score_mine = np.zeros(1)
            ...
            for i, data in tqdm(enumerate(val_data), desc="Processing", total=num):
                ...
                psnr = -10 * math.log10(
                    torch.mean((gt[2] - pred[0]) * (gt[2] - pred[0])).cpu().data)
                ssim_val    = ssim(gt[2:3], pred[0:1],
                                   data_range=1.0, size_average=False)
                ms_ssim_val = ms_ssim(gt[2:3], pred[0:1],
                                      data_range=1.0, size_average=False)
                x = ((gt[2:3]   - 0.5) * 2.0).clone()
                y = ((pred[0:1] - 0.5) * 2.0).clone()
                lpips_val = loss_fn_alex(x, y)
                ...
                if save_img:
                    ...
                    cv2.imwrite(os.path.join(pred_1_name, 'pred_1.png'), pred_1[0])
